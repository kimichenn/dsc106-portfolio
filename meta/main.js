...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: 'https://github.com/kimichenn/dsc106-portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
function renderCommitInfo(data, commits) {
  ...
  dl.append('dd').text(d3.format('.1f')(averageFileLength));

  dl.append('dt').text('Average line length');
  dl.append('dd').text(d3.format('.1f')(d3.mean(data, (d) => d.length)));
}
...
// Start both scrollytelling vizes at the first commit so they grow as you
// scroll, instead of flashing the full repo before the first step triggers.
const initialCommits = commits.filter((d) => d.datetime <= commits[0].datetime);
updateScatterPlot(data, initialCommits);
updateFileDisplay(initialCommits);
